...
The helper writes B's configuration into `firmware/config/internal/board_control.toml` and the transmitter settings into `transmit/config.toml`. The expected build output is `firmware/build/receive-endpoint/manifest.json`, with `endpoint_id: 2` and `compiled: true`. Upload must finish successfully with flash verification.
...
- Arduino AVR Boards 1.8.8 is installed; Python 3.12 and the help commands work.
...
Continue with the [transmission guide](../../transmit/README.md) or the [hardware checks](../../tests/README.md). Start the receiver before confirming transmission on the Pi. After reboot, activate the environment and return to the repository; repeat setup only for changed software or hardware.
