...
def configure(root, *, device, board, role, doppler=None, count=None, profile=None):
    ...
    board_file = 'firmware/config/internal/transmit_endpoint.toml'
    build = 'firmware/build/single-endpoint'
    if board == 'B':
        board_file = 'firmware/config/internal/receive_endpoint.toml'
        build = 'firmware/build/receive-endpoint'
    maintenance = root / 'firmware/config/internal/board_control.toml'
    ...
    if role == 'tx':
        ...
        if doppler == 'off':
            ...
            values['orbit_dataset'] = 'dataset/data/replay/gomx1-example/orbit'
    ...
    entry = main.parent.relative_to(root) / 'main.py'
    print(f'Configuration saved. After verifying the flashed build: python {entry.as_posix()}')
