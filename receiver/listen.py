...
def run(arguments=None) -> int:
    ...
    truth = load_profile_truth(PROJECT_ROOT / "firmware" / "config")
    profile_ids = []
    for profile in truth.profiles:
        profile_ids.append(profile.profile_id)
    if options.profile not in profile_ids:
        parser.error("profile must match a ProfileId in firmware/config/profiles.csv")
    ...
    for name in ("profiles.csv", "pa_settings.csv", "profile_registers.csv"):
        source = PROJECT_ROOT / "firmware" / "config" / name
    ...
    try:
        ...
        configure_endpoint(transport, truth, manifest, options.profile, 2, options.timeout_ms)
