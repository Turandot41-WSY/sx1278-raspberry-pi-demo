...
This selects `config/internal/receive_endpoint.toml` and B's `firmware/build/receive-endpoint/manifest.json`.
...
This selects `config/internal/transmit_endpoint.toml` and A's `firmware/build/single-endpoint/manifest.json`.
...
- Python 3.12 and AVR Boards 1.8.8 are selected; all software tests pass.
- The intended COM port and A/B board are configured; build and upload succeed.

Continue with the [antenna experiment](../../experiments/README.md). A Windows sender runs `python main_transmit.py`; a Windows receiver runs `python main_receive.py`. In Windows → Windows, these commands run on different computers with separate local serial ports.
